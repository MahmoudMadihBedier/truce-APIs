...
export class AmazonScraper extends BaseScraper {
  ...
  private async launchBrowser(): Promise<Browser> {
    ...
    // Add --disable-http2 to mitigate ERR_HTTP2_PROTOCOL_ERROR on some networks/proxies
  ...
  public parseProduct($el: cheerio.Cheerio<Element>): Product | null {
    try {
      const name = ($el.find('h2 span').text() || $el.find('.a-link-normal .a-text-normal').text()).trim();
